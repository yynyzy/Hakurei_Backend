...
#[get("/")]
pub async fn get_all_users(_auth_guard: auth::BasicAuth) -> ApiResponse<Vec<UserModel>> {
...
) -> Result<ApiResponse<ResponseTokenStruct>, ApiResponse<()>> {
...
) -> ApiResponse<ResponseTokenStruct> {
    print!("{:?}", login_params);
